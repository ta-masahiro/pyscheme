;(define R5RS-macro_VER "10312256:define-syntax/case-lambda/let-optionals*")
;(display "\n\tR5RS-macro     version: ")
;(display R5RS-macro_VER)

;;;; "mbe.scm" "Macro by Example" (Eugene Kohlbecker, R4RS)
;;; From: Dorai Sitaram, dorai@cs.rice.edu, 1991, 1999
;;; changed for SECD_py by M. Taniguro 2016
;
...
;   corr., Oct. 15, 2016. Masahiro Tniguro
...
(define hyg:rassq
  ; (hyg:rassq 3 '((a . 1) (b . 2) (c . 3) (d . 4)))
  ; => (c . 3)
  ; (hyg:rassq 4 '((a . 1) (b . 2) (c . 3) (d . 4)))
  ; => #f
  (lambda (k al)
    (let loop ((al al))
      (if (null? al) #f
...
(define gentemp gensym)     ; gentempの代わりにgensymを使う
...
;(define (some pred . lists)
;  (:some (get-python-function pred) lists))

;(define (every pred . lists)
;  (:every (get-python-function pred) lists))
(define some any)
...
;@
; (define macro:eval defmacro:eval)     ; 使っていない
; (define macro:load defmacro:load)     ; 使っていない
; (define macro:expand defmacro:expand*); 使っていない
(define slib:error error )
;@ define-syntax
(define-macro define-syntax (lambda (macro-name syn-rules)
  (if (or (not (pair? syn-rules))
	...
	`(define-macro ,macro-name (lambda macro-arg
...
;;;
;;;
(let-macro let-syntax (lambda (defs . bodys)
  (if (or (not (pair? syn-rules))
	  (not (eq? (car syn-rules) 'syntax-rules)))
      (slib:error 'define-syntax 'not-an-r4rs-high-level-macro
		  macro-name syn-rules)
      (let ((keywords (cons macro-name (cadr syn-rules)))
	    (clauses (cddr syn-rules)))
	`(let-macro (,macro-name (lambda macro-arg
	   (let ((macro-arg (cons ',macro-name macro-arg))
		 (keywords ',keywords))
	     (cond ,@(map
		      (lambda (clause)

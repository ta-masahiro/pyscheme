...
(define $r6rs-eval                 #f)
(define $expand-toplevel-sequence  #f)
...
(define (memp proc ls)
  (:memp (get-python-function proc) ls))
...
;; Start of expander:
 ...
 (let ()   ; ###########################################################
  ...
  (set! $repl                      repl)
  (set! $expand-toplevel-sequence  expand-toplevel-sequence)
  (set! $syntax-violation          syntax-violation)
   ) ; Expander
...
(define (macro:expand exp) 
  (let ((expand-list ($expand-toplevel-sequence (list exp))))
    (if (null? expand-list) '()
        (car expand-list))))

(print "Expand: macros-core")

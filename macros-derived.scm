...
(library (core primitives)
   ...
   * + - / < <= = > >= abs acos append apply asin atan 
   ...
   ;; pysheme original functions

   1+ 1- exit quit macro:expand :eval :load require list-copy append! reverse! time list=
   list* cons* :member :assoc map-car map-cdr print :list-tabulate square range last-pair
   circular-list? :count get-python-function compile vm ^ :reduce :map! :unfold fold any every
   rnrs:vector-fill! make-hash-table hash-table-exists? hash-table-get hash-table-put!
   timeit vector-fold vector-fold-right vector-unfold  vector-unfold-right range-v string-map
